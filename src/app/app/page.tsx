...
import { getCurrentUser, logout, listDocuments, createDocument, COLLECTION_IDS } from "@/lib/appwrite";
...
function BotsTab() {
  ...
  useEffect(() => {
    async function fetchBots() {
      setLoading(true);
      setError('');
      const res = await listDocuments('default', COLLECTION_IDS.agents, []);
  ...
  const handleAddBot = async (e: React.FormEvent) => {
    ...
    // Add bot to Appwrite
    const res = await createDocument('default', COLLECTION_IDS.agents, data);
    if (res instanceof Error) {
      setError(res.message);
      return;
    }
